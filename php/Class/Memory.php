...
class Memory{
	...
	public function __construct($workerId, $task){
		//$this->role = $role;
		$this->dbh = new DB();
		$this->task = $task;
	...
	*/
	...
	public function postMemory($memoryLine){
		...
		*/
		...
		$memoryId = $this->dbh->index("memory", $propArray);
		return $memoryId;
		/*
		if($this->$speakerId){
	...
	public function fetchNewMemory($lastMemoryId){
	
		$propArray = array(
				"task" => $this->task
		);
	
		$newLastMemoryId = $this->dbh->getLastId("memory", $propArray);
		if($newLastMemoryId>$lastMemoryId){
			$newMemories = $this->dbh->getRowsBeginFrom("memory", $propArray, $lastMemoryId+1);
			return $newMemories;
		}else{
			return false;
		}
	...
	function link_it($text){
		...
		return preg_replace('!(((f|ht)tp(s)?://)[-a-zA-Z訄-��-觓()0-9@:%_+.~#?&;//=]+)!i', '<a target="_blank" href="$1">$1</a>', $text);
		//}
